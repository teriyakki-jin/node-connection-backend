...
function createOrgs() {
  ...
  if [ "$CRYPTO" == "cryptogen" ]; then
    ...
    infoln "Creating Registry Identities"

    set -x
    cryptogen generate --config=./organizations/cryptogen/crypto-config-registry.yaml --output="organizations"
    res=$?
    { set +x; } 2>/dev/null
    if [ $res -ne 0 ]; then
    ...
    infoln "Creating Viewer Identities"

    set -x
    cryptogen generate --config=./organizations/cryptogen/crypto-config-viewer.yaml --output="organizations"
    res=$?
    { set +x; } 2>/dev/null
    if [ $res -ne 0 ]; then
      fatalln "Failed to generate certificates..."
  ...
  if [ "$CRYPTO" == "cfssl" ]; then
    ...
    peer_cert peer peer0.registry.node.connection registry
    peer_cert admin Admin@registry.node.connection registry

    infoln "Creating Viewer Identities"
    #function_name cert-type   CN   org
    peer_cert peer peer0.viewer.node.connection viewer
    peer_cert admin Admin@viewer.node.connection viewer
  ...
  if [ "$CRYPTO" == "Certificate Authorities" ]; then
    ...
    while :
    do
      if [ ! -f "organizations/fabric-ca/registry/tls-cert.pem" ]; then
        sleep 1
      else
        break
      fi
  ...
  infoln "Generating CCP files for Registry and Viewer"
...
# call the script to create the channel, join the peers of registry and viewer,
...
function networkDown() {
  ...
  # stop org3 containers also in addition to registry and viewer, in case we were running sample to add org3
  ...
  if [ "$MODE" != "restart" ]; then
    ...
    ${CONTAINER_CLI} volume rm docker_orderer.node.connection docker_peer0.registry.node.connection docker_peer0.viewer.node.connection
    ...
    ${CONTAINER_CLI} run --rm -v "$(pwd):/data" busybox sh -c 'cd /data && rm -rf organizations/fabric-ca/registry/msp organizations/fabric-ca/registry/tls-cert.pem organizations/fabric-ca/registry/ca-cert.pem organizations/fabric-ca/registry/IssuerPublicKey organizations/fabric-ca/registry/IssuerRevocationPublicKey organizations/fabric-ca/registry/fabric-ca-server.db'
    ${CONTAINER_CLI} run --rm -v "$(pwd):/data" busybox sh -c 'cd /data && rm -rf organizations/fabric-ca/viewer/msp organizations/fabric-ca/viewer/tls-cert.pem organizations/fabric-ca/viewer/ca-cert.pem organizations/fabric-ca/viewer/IssuerPublicKey organizations/fabric-ca/viewer/IssuerRevocationPublicKey organizations/fabric-ca/viewer/fabric-ca-server.db'
