...
function peer_cert() {
    ...
    mkdir -p "organizations/peerOrganizations/$ORG.node.connection/ca"
    mkdir -p "organizations/peerOrganizations/$ORG.node.connection/msp/cacerts"
    mkdir -p "organizations/peerOrganizations/$ORG.node.connection/msp/tlscacerts"
    mkdir -p "organizations/peerOrganizations/$ORG.node.connection/peers"
    mkdir -p "organizations/peerOrganizations/$ORG.node.connection/tlsca"

    CERT_DIR=organizations/peerOrganizations/$ORG.node.connection

    if [ ! -f "$CERT_DIR/ca/ca-key.pem" ]; then

        cfssl gencert -initca "${PWD}/organizations/cfssl/ca-peer.json" | cfssljson -bare "$CERT_DIR/ca/ca"

        cp "$CERT_DIR/ca/ca.pem" "$CERT_DIR/tlsca/tlsca.$ORG.node.connection-cert.pem"
        cp "$CERT_DIR/ca/ca.pem" "$CERT_DIR/ca/ca.$ORG.node.connection-cert.pem"
...
function orderer_cert() {
    TYPE=$1 #orderer user
    USER=$2 #orderer.node.connection

    mkdir -p organizations/ordererOrganizations/node.connection/ca
    mkdir -p organizations/ordererOrganizations/node.connection/msp/cacerts
    mkdir -p organizations/ordererOrganizations/node.connection/msp/tlscacerts
    mkdir -p organizations/ordererOrganizations/node.connection/orderers
    mkdir -p organizations/ordererOrganizations/node.connection/tlsca

    CERT_DIR=organizations/ordererOrganizations/node.connection

    if [ ! -f "$CERT_DIR/ca/ca-key.pem" ]; then

        cfssl gencert -initca "${PWD}/organizations/cfssl/ca-orderer.json" | cfssljson -bare "$CERT_DIR/ca/ca"

        cp "$CERT_DIR/ca/ca.pem" "$CERT_DIR/tlsca/tlsca.node.connection-cert.pem"

        cp "$CERT_DIR/ca/ca.pem" "$CERT_DIR/msp/cacerts/"
        cp "$CERT_DIR/ca/ca.pem" "$CERT_DIR/msp/tlscacerts/tlsca.node.connection-cert.pem"
...
function generate_orderer_certs() {
    ...
    for DIR in cacerts keystore signcerts tlscacerts; do
        mkdir -p "organizations/ordererOrganizations/node.connection/orderers/$USER/msp/$DIR"
    done

    mkdir -p "organizations/ordererOrganizations/node.connection/orderers/$USER/tls"
    ...
    cp "$CERT_DIR/ca/ca.pem" "$CERT_DIR/orderers/$USER/msp/tlscacerts/tlsca.node.connection-cert.pem"
